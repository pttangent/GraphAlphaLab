...
def load_checkpoint_frames(root: str | Path, names: Iterable[str]) -> dict[str, pd.DataFrame]:
    path = Path(root)
    result: dict[str, pd.DataFrame] = {}
    for name in names:
        file_path = path / name
        result[name] = pd.read_parquet(file_path) if file_path.exists() else pd.DataFrame()
    return result
