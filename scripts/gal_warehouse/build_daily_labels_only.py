from __future__ import annotations

import argparse
import json
import os
from pathlib import Path
import shutil

import duckdb
import pandas as pd

from graphalphalab.checkpoint import CheckpointSpec, checkpoint_valid, file_signature, utc_now, write_progress
from graphalphalab.governance import atomic_write_frame, atomic_write_json, file_record, sha256_file, sha256_json


BASE = Path(r"D:\DEV\AnotherNetworkFactory")
DEFAULT_BARS_ROOT = BASE / "warehouses" / "NFF_warehouse" / "canonical" / "bars_1m" / "schema=v1"
DEFAULT_OUT = BASE / "warehouses" / "GAL_warehouse"

EXPECTED_DATES = [
    "2026-06-01", "2026-06-02", "2026-06-03", "2026-06-04", "2026-06-05",
    "2026-06-08", "2026-06-09", "2026-06-10", "2026-06-11", "2026-06-12",
    "2026-06-15", "2026-06-16", "2026-06-17", "2026-06-18", "2026-06-22",
    "2026-06-23", "2026-06-24", "2026-06-25", "2026-06-26", "2026-06-29",
    "2026-06-30", "2026-07-01", "2026-07-02", "2026-07-06", "2026-07-07",
    "2026-07-08", "2026-07-09", "2026-07-10", "2026-07-13", "2026-07-14",
    "2026-07-15", "2026-07-16", "2026-07-17",
]


LABEL_CONTRACT_TEMPLATE = {
    "horizon_minutes": 1,
    "entry_lag_minutes": 1,
    "target_column": "target_return",
    "decision_time_column": "decision_time",
    "entry_time_column": "entry_time",
    "exit_time_column": "exit_time",
    "available_time_column": "label_available_time",
    "overlapping": True,
    "rebalance_minutes": None,
    "horizon_tolerance_seconds": 21 * 24 * 3600,
    "require_entry_after_decision": True,
}


def sql_path(path: Path) -> str:
    return path.expanduser().resolve().as_posix().replace("'", "''")


def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(description="Build date-checkpointed daily forward labels")
    result.add_argument("--bars-root", type=Path, default=DEFAULT_BARS_ROOT)
    result.add_argument("--out-root", type=Path, default=DEFAULT_OUT)
    result.add_argument("--threads", type=int, default=8)
    result.add_argument("--memory-limit-gb", type=float, default=24.0)
    result.add_argument("--reset-checkpoints", action="store_true")
    return result


def discover_bar_files(bars_root: Path) -> dict[str, list[Path]]:
    result: dict[str, list[Path]] = {}
    for trade_date in EXPECTED_DATES:
        date_root = bars_root / f"date={trade_date}"
        files = sorted(date_root.rglob("part-*.parquet")) if date_root.exists() else []
        if not files:
            raise FileNotFoundError(f"Missing canonical bars for {trade_date}: {date_root}")
        result[trade_date] = files
    return result


def write_daily_contract(contracts: Path, label_id: str) -> None:
    atomic_write_json(contracts / f"{label_id}.json", {"label_id": label_id, **LABEL_CONTRACT_TEMPLATE})


def source_records_by_date(files_by_date: dict[str, list[Path]]) -> dict[str, list[dict[str, object]]]:
    return {
        trade_date: [file_record(path) for path in paths]
        for trade_date, paths in files_by_date.items()
    }


def shard_source_records(
    all_records: dict[str, list[dict[str, object]]],
    base_index: int,
) -> list[dict[str, object]]:
    records: list[dict[str, object]] = []
    for trade_date in EXPECTED_DATES[base_index : base_index + 8]:
        records.extend(all_records[trade_date])
    return records


def write_shard_checkpoint(
    shard_root: Path,
    spec: CheckpointSpec,
    *,
    output_rows: int,
    source_records: list[dict[str, object]],
) -> None:
    record = file_signature(shard_root / "data.parquet")
    record["path"] = "data.parquet"
    record["rows"] = int(output_rows)
    atomic_write_json(
        shard_root / "checkpoint.json",
        {
            **spec.as_dict(),
            "status": "complete",
            "completed_at": utc_now(),
            "files": [record],
            "source_inputs": source_records,
        },
    )


def configure(connection: duckdb.DuckDBPyConnection, *, threads: int, memory_limit_gb: float) -> None:
    connection.execute(f"PRAGMA threads={max(1, int(threads))}")
    connection.execute(f"SET memory_limit='{float(memory_limit_gb):g}GB'")
    connection.execute("PRAGMA preserve_insertion_order=false")


def create_source_views(connection: duckdb.DuckDBPyConnection, bars_root: Path) -> None:
    pattern = sql_path(bars_root / "date=*" / "part-*.parquet")
    date_values = ", ".join(f"'{trade_date}'" for trade_date in EXPECTED_DATES)
    connection.execute(
        f"""
        CREATE OR REPLACE TEMP VIEW bars AS
        SELECT
          CAST(trade_date AS VARCHAR) AS trade_date,
          CAST(symbol_id AS BIGINT) AS symbol_id,
          CAST(symbol AS VARCHAR) AS symbol,
          CAST(timestamp AS TIMESTAMPTZ) AS timestamp,
          CAST(available_time AS TIMESTAMPTZ) AS available_time,
          CAST(close AS DOUBLE) AS close,
          CAST(open AS DOUBLE) AS open
        FROM read_parquet('{pattern}', union_by_name=true, hive_partitioning=false)
        WHERE trade_date IN ({date_values})
          AND symbol_id IS NOT NULL
          AND close IS NOT NULL
          AND open IS NOT NULL
        """
    )
    connection.execute(
        """
        CREATE OR REPLACE TEMP VIEW rth_bars AS
        SELECT *
        FROM bars
        WHERE strftime(timestamp, '%H:%M:%S') >= '21:30:00'
           OR strftime(timestamp, '%H:%M:%S') <= '04:00:00'
        """
    )
    connection.execute(
        """
        CREATE OR REPLACE TEMP VIEW daily_ohlc AS
        SELECT
          trade_date,
          symbol_id,
          any_value(symbol) AS symbol,
          min(timestamp) AS rth_open_time,
          max(timestamp) AS rth_close_time,
          arg_min(open, timestamp) AS rth_open_price,
          arg_max(close, timestamp) AS rth_close_price,
          arg_min(available_time, timestamp) AS open_available_time,
          arg_max(available_time, timestamp) AS close_available_time,
          count(*) AS rth_bar_count
        FROM rth_bars
        GROUP BY trade_date, symbol_id
        HAVING rth_open_price IS NOT NULL AND rth_close_price IS NOT NULL
        """
    )
    connection.execute(
        """
        CREATE OR REPLACE TEMP VIEW calendar AS
        SELECT trade_date, row_number() OVER (ORDER BY trade_date) - 1 AS session_index
        FROM (SELECT DISTINCT trade_date FROM daily_ohlc)
        """
    )


def shard_query(base_date: str) -> str:
    return f"""
      WITH entries AS (
        SELECT c.trade_date, c.session_index, d.symbol_id,
               'n_close' AS entry_mode,
               d.rth_close_time AS entry_time,
               d.rth_close_price AS entry_price,
               d.close_available_time AS entry_available_time
        FROM daily_ohlc d JOIN calendar c USING (trade_date)
        WHERE d.trade_date='{base_date}'
        UNION ALL
        SELECT c.trade_date, c.session_index, d.symbol_id,
               'n1_open' AS entry_mode,
               n.rth_open_time AS entry_time,
               n.rth_open_price AS entry_price,
               n.open_available_time AS entry_available_time
        FROM daily_ohlc d
        JOIN calendar c ON c.trade_date=d.trade_date
        JOIN calendar cn ON cn.session_index=c.session_index+1
        JOIN daily_ohlc n ON n.trade_date=cn.trade_date AND n.symbol_id=d.symbol_id
        WHERE d.trade_date='{base_date}'
        UNION ALL
        SELECT c.trade_date, c.session_index, d.symbol_id,
               'n1_close' AS entry_mode,
               n.rth_close_time AS entry_time,
               n.rth_close_price AS entry_price,
               n.close_available_time AS entry_available_time
        FROM daily_ohlc d
        JOIN calendar c ON c.trade_date=d.trade_date
        JOIN calendar cn ON cn.session_index=c.session_index+1
        JOIN daily_ohlc n ON n.trade_date=cn.trade_date AND n.symbol_id=d.symbol_id
        WHERE d.trade_date='{base_date}'
      ),
      exit_points AS (
        SELECT c.trade_date, c.session_index, d.symbol_id,
               'open' AS exit_point,
               d.rth_open_time AS exit_time,
               d.rth_open_price AS exit_price,
               d.open_available_time AS exit_available_time
        FROM daily_ohlc d JOIN calendar c ON c.trade_date=d.trade_date
        UNION ALL
        SELECT c.trade_date, c.session_index, d.symbol_id,
               'close' AS exit_point,
               d.rth_close_time AS exit_time,
               d.rth_close_price AS exit_price,
               d.close_available_time AS exit_available_time
        FROM daily_ohlc d JOIN calendar c ON c.trade_date=d.trade_date
      )
      SELECT
        base.trade_date,
        rb.timestamp AS decision_time,
        base.symbol_id,
        'daily_' || e.entry_mode || '_to_n' || CAST(x.session_index - e.session_index AS VARCHAR) || '_' || x.exit_point AS label_id,
        e.entry_time,
        x.exit_time,
        greatest(e.entry_available_time, x.exit_available_time) AS label_available_time,
        e.entry_price,
        x.exit_price,
        x.exit_price / nullif(e.entry_price, 0) - 1.0 AS target_return,
        e.entry_mode,
        x.exit_point,
        CAST(x.session_index - e.session_index AS INTEGER) AS exit_session_offset
      FROM daily_ohlc base
      JOIN rth_bars rb ON rb.trade_date=base.trade_date AND rb.symbol_id=base.symbol_id
      JOIN calendar cb ON cb.trade_date=base.trade_date
      JOIN entries e ON e.trade_date=base.trade_date AND e.symbol_id=base.symbol_id
      JOIN exit_points x
        ON x.symbol_id=base.symbol_id
       AND x.session_index BETWEEN cb.session_index + 1 AND cb.session_index + 7
      WHERE base.trade_date='{base_date}'
        AND e.entry_time > rb.timestamp
        AND x.exit_time > e.entry_time
        AND x.exit_price IS NOT NULL
        AND e.entry_price IS NOT NULL
    """


def main() -> None:
    args = parser().parse_args()
    bars_root = args.bars_root.expanduser().resolve()
    out = args.out_root.expanduser().resolve()
    labels = out / "labels"
    contracts = out / "contracts"
    diag = out / "label_diagnostics"
    parts = labels / "daily_forward_parts"
    labels.mkdir(parents=True, exist_ok=True)
    contracts.mkdir(parents=True, exist_ok=True)
    diag.mkdir(parents=True, exist_ok=True)
    if args.reset_checkpoints and parts.exists():
        shutil.rmtree(parts)
    parts.mkdir(parents=True, exist_ok=True)

    files_by_date = discover_bar_files(bars_root)
    records_by_date = source_records_by_date(files_by_date)
    code_sha = sha256_file(Path(__file__))
    contract_hash = sha256_json(
        {
            "operation": "daily_forward_labels_date_checkpointed",
            "code_sha256": code_sha,
            "expected_dates": EXPECTED_DATES,
            "label_contract": LABEL_CONTRACT_TEMPLATE,
            "rth_window": "21:30:00-04:00:00 Asia/Taipei rendering of stored TIMESTAMPTZ",
            "max_exit_session_offset": 7,
        }
    )

    connection = duckdb.connect()
    configure(connection, threads=args.threads, memory_limit_gb=args.memory_limit_gb)
    create_source_views(connection, bars_root)
    states: list[dict[str, object]] = []
    completed = 0
    reused = 0
    total = len(EXPECTED_DATES)
    write_progress(diag, stage="daily-label-date-checkpoints", total=total, completed=0, units=[])

    try:
        for index, trade_date in enumerate(EXPECTED_DATES):
            shard_root = parts / f"date={trade_date}"
            shard_root.mkdir(parents=True, exist_ok=True)
            source_records = shard_source_records(records_by_date, index)
            source_hash = sha256_json(source_records)
            spec = CheckpointSpec("daily-label-date", trade_date, contract_hash, source_hash)
            state = {"unit": trade_date, "status": "pending", "attempt": 0, "detail": str(shard_root)}
            states.append(state)
            if checkpoint_valid(shard_root, spec, required_files=("data.parquet",)):
                state["status"] = "reused"
                reused += 1
                completed += 1
                write_progress(
                    diag,
                    stage="daily-label-date-checkpoints",
                    total=total,
                    completed=completed,
                    reused=reused,
                    current=trade_date,
                    units=states,
                )
                continue

            state["status"] = "running"
            state["attempt"] = 1
            write_progress(
                diag,
                stage="daily-label-date-checkpoints",
                total=total,
                completed=completed,
                reused=reused,
                current=trade_date,
                units=states,
            )
            destination = shard_root / "data.parquet"
            temporary = shard_root / f".data.parquet.{os.getpid()}.part"
            destination.unlink(missing_ok=True)
            (shard_root / "checkpoint.json").unlink(missing_ok=True)
            temporary.unlink(missing_ok=True)
            connection.execute(
                f"COPY ({shard_query(trade_date)}) TO '{sql_path(temporary)}' "
                "(FORMAT PARQUET, COMPRESSION ZSTD, ROW_GROUP_SIZE 100000)"
            )
            os.replace(temporary, destination)
            rows = int(connection.execute(f"SELECT count(*) FROM read_parquet('{sql_path(destination)}')").fetchone()[0])
            write_shard_checkpoint(
                shard_root,
                spec,
                output_rows=rows,
                source_records=source_records,
            )
            state["status"] = "complete"
            state["detail"] = f"{rows} rows"
            completed += 1
            write_progress(
                diag,
                stage="daily-label-date-checkpoints",
                total=total,
                completed=completed,
                reused=reused,
                current=trade_date,
                units=states,
            )

        daily_dest = labels / "daily_forward.parquet"
        temp_dest = labels / f".daily_forward.parquet.{os.getpid()}.part"
        temp_dest.unlink(missing_ok=True)
        parts_pattern = sql_path(parts / "date=*" / "data.parquet")
        connection.execute(
            f"COPY (SELECT * FROM read_parquet('{parts_pattern}', union_by_name=true, hive_partitioning=false)) "
            f"TO '{sql_path(temp_dest)}' (FORMAT PARQUET, COMPRESSION ZSTD, ROW_GROUP_SIZE 100000)"
        )

        coverage = connection.execute(
            f"""
            SELECT
              label_id,
              any_value(entry_mode) AS entry_mode,
              any_value(exit_point) AS exit_point,
              any_value(exit_session_offset) AS exit_session_offset,
              count(*) AS label_rows,
              count(DISTINCT trade_date) AS trade_dates,
              count(DISTINCT decision_time) AS decision_times,
              count(DISTINCT symbol_id) AS symbols
            FROM read_parquet('{sql_path(temp_dest)}')
            GROUP BY label_id
            ORDER BY entry_mode, exit_session_offset, exit_point
            """
        ).df()
        rth = connection.execute(
            f"""
            SELECT
              count(*) AS row_count,
              sum(CASE
                    WHEN strftime(decision_time, '%H:%M:%S') >= '21:30:00'
                      OR strftime(decision_time, '%H:%M:%S') <= '04:00:00'
                    THEN 0 ELSE 1
                  END) AS invalid_decision_rows,
              min(strftime(decision_time, '%H:%M:%S')) AS min_decision_time,
              max(strftime(decision_time, '%H:%M:%S')) AS max_decision_time,
              count(DISTINCT label_id) AS label_count,
              count(DISTINCT trade_date) AS trade_date_count
            FROM read_parquet('{sql_path(temp_dest)}')
            """
        ).fetchone()
        rth_check = {
            "row_count": int(rth[0] or 0),
            "invalid_decision_rows": int(rth[1] or 0),
            "min_decision_time": rth[2],
            "max_decision_time": rth[3],
            "label_count": int(rth[4] or 0),
            "trade_date_count": int(rth[5] or 0),
            "rth_window_local_timestamp_rendering": "21:30:00-04:00:00 Asia/Taipei display for stored TIMESTAMPTZ",
        }
        if rth_check["invalid_decision_rows"]:
            temp_dest.unlink(missing_ok=True)
            raise RuntimeError(f"daily_forward.parquet failed RTH decision_time validation: {rth_check}")
        os.replace(temp_dest, daily_dest)

        atomic_write_frame(coverage, diag / "daily_forward_coverage.csv")
        atomic_write_json(diag / "daily_forward_rth_verified.json", rth_check)
        label_ids = sorted(coverage["label_id"].astype(str).unique().tolist())
        for label_id in label_ids:
            write_daily_contract(contracts, label_id)
        manifest = {
            "source": str(bars_root),
            "expected_dates": EXPECTED_DATES,
            "daily_label_count": len(label_ids),
            "daily_labels": label_ids,
            "rth_check": rth_check,
            "price_source": "bars_1m close/open",
            "tail_missing_policy": "label_id coverage is reported separately; unavailable N+offset exits are absent from that label_id denominator",
            "checkpoint_granularity": "base_trade_date",
            "checkpoint_count": total,
            "checkpoint_reused": reused,
            "checkpoint_contract_hash": contract_hash,
            "final_output": file_record(daily_dest),
        }
        atomic_write_json(diag / "daily_label_manifest.json", manifest)
        atomic_write_json(diag / "daily_forward_SUCCESS.json", {"status": "complete", "contract_hash": contract_hash})
        write_progress(
            diag,
            stage="daily-label-date-checkpoints",
            total=total,
            completed=total,
            reused=reused,
            status="complete",
            units=states,
            extra={"final_output": str(daily_dest), "contract_hash": contract_hash},
        )
        print(json.dumps(manifest, indent=2, default=str))
    except Exception as exc:
        if states and states[-1].get("status") == "running":
            states[-1]["status"] = "failed"
            states[-1]["detail"] = repr(exc)
        write_progress(
            diag,
            stage="daily-label-date-checkpoints",
            total=total,
            completed=completed,
            reused=reused,
            failed=1,
            current=states[-1]["unit"] if states else None,
            status="failed",
            units=states,
            extra={"error": repr(exc), "contract_hash": contract_hash},
        )
        raise
    finally:
        connection.close()


if __name__ == "__main__":
    main()
