...
1. **Preflight decision inventory**
   ...
   - Reuses the cache only when the export anchor and requested date range match.
2. **Forward-volatility label DAG**
   - One atomic checkpoint per trading date.
   - Reads only `timestamp`, `symbol_id`, and `ret_1m` from NFF `gff_core`.
   - Builds 30/60/120/180-minute forward labels after a one-minute entry lag.
   - Validates duplicate keys, entry/exit ordering, label availability, exact non-truncated tails, and minimum observation coverage.
...
3. **Contract publication**
   ...
   - The contract checkpoint is anchored to all daily label-file fingerprints.
...
Re-run the identical command after interruption. Valid decision caches, date-label checkpoints, horizon preflight caches and factor checkpoints are reused. Only incomplete units are recomputed.
